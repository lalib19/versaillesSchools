...
const Schools = () => {
  ...
  if (filter === 'All') {
    ...
    schoollist = data.filter(
      isfiltered => isfiltered.record.fields.type_etablissement == filterused,
    );
  }

  let schoollist1;
  if (filter !== 'All') {
    schoollist1 = schoollist.filter(
      isfiltered => isfiltered.record.fields.statut_public_prive == filtertype,
    );
  } else {
    ...
    <View style={{flex: 1, padding: 5}}>
      ...
      <FlatList
        ...
        keyExtractor={(item, index) =>
          item.record.fields.identifiant_de_l_etablissement
        }
        renderItem={({item}) => (
          <Card
